...
D_DEBUG_DOMAIN( Prophecy_Window, "Prophecy/Window", "Prophecy's Window" );


static const ReactionFunc prophecy_window_globals[] = {
     _prophecy_wm_module_window_listener,
     NULL
};
...
window_destructor( FusionObject *object, bool zombie, void *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Window, "destroying %p (%dx%d - %dx%d)%s\n",
     ...
     prophecy_window_notify( window, UWNF_DESTROYED );
     ...
     prophecy_input_switch_drop( context->input_switch, window->channel );

     prophecy_input_channel_detach_global( window->channel, &window->channel_reaction );

     prophecy_input_channel_destroy( window->channel );


     if (window->surface)
          dfb_surface_unlink( &window->surface );

     prophecy_context_unlink( &window->context );
...
FusionObjectPool *
prophecy_window_pool_create( const FusionWorld *world )
{
     return fusion_object_pool_create( "Prophecy Window Pool", sizeof(UniqueWindow),
...
DFBResult
prophecy_window_create( CoreDFB                 *core,
                      CoreWindow              *window,
                      UniqueContext           *context,
                      ...
                      UniqueWindow           **ret_window )
{
     ...
     uniwin = prophecy_wm_create_window();
     if (!uniwin)
          return DFB_FUSION;
     ...
     ret = prophecy_context_link( &uniwin->context, context );
     if (ret)
          goto error;
     ...
     ret = prophecy_input_channel_create( core, context, &uniwin->channel );
     if (ret)
          goto error;

     ret = prophecy_input_channel_attach_global( uniwin->channel,
                                               PROPHECY_WINDOW_INPUT_CHANNEL_LISTENER,
     ...
     //prophecy_context_update_focus( context );
...
error:
     if (uniwin->channel_reaction.attached)
          prophecy_input_channel_detach_global( uniwin->channel, &uniwin->channel_reaction );

     if (uniwin->channel)
          prophecy_input_channel_destroy( uniwin->channel );

     if (uniwin->surface)
          dfb_surface_unlink( &uniwin->surface );

     if (uniwin->context)
          prophecy_context_unlink( &uniwin->context );
...
DFBResult
prophecy_window_close( UniqueWindow *window )
{
     D_MAGIC_ASSERT( window, UniqueWindow );
...
DFBResult
prophecy_window_destroy( UniqueWindow *window )
{
     D_MAGIC_ASSERT( window, UniqueWindow );
...
DFBResult
prophecy_window_notify( UniqueWindow                  *window,
                      UniqueWindowNotificationFlags  flags )
{
     ...
     return prophecy_window_dispatch( window, &notification, prophecy_window_globals );
}

DFBResult
prophecy_window_update( UniqueWindow        *window,
                      const DFBRegion     *region,
                      DFBSurfaceFlipFlags  flags )
{
     D_MAGIC_ASSERT( window, UniqueWindow );
...
DFBResult
prophecy_window_post_event( UniqueWindow   *window,
...
DFBResult
prophecy_window_set_config( UniqueWindow           *window,
                          const CoreWindowConfig *config,
...
DFBResult
prophecy_window_get_config( UniqueWindow     *window,
...
DFBResult
prophecy_window_restack( UniqueWindow *window,
                       UniqueWindow *relative,
                       int           relation )
{
     D_MAGIC_ASSERT( window, UniqueWindow );
...
DFBResult
prophecy_window_grab( UniqueWindow     *window,
                    const CoreWMGrab *grab )
{
     ...
     switch (grab->target) {
          case CWMGT_KEYBOARD:
               return prophecy_input_switch_set( context->input_switch,
          ...
          case CWMGT_KEY: {
               ...
               ret = prophecy_input_switch_set_filter( context->input_switch, UDCI_KEYBOARD,
                                                     window->channel, &event, &filter );
               if (ret) {
                    D_DERROR( ret, "Prophecy/Window: Could not set input filter for key grab!\n" );
                    return ret;
               }

               ret = add_key_filter( window, grab->symbol, grab->modifiers, filter );
               if (ret) {
                    prophecy_input_switch_unset_filter( context->input_switch, filter );
                    return ret;
               }
...
DFBResult
prophecy_window_ungrab( UniqueWindow     *window,
                      const CoreWMGrab *grab )
{
     UniqueContext *context;
     ...
     switch (grab->target) {
          case CWMGT_KEYBOARD:
               return prophecy_input_switch_unset( context->input_switch,
                                                 UDCI_KEYBOARD, window->channel );
...
DFBResult
prophecy_window_request_focus( UniqueWindow *window )
{
     UniqueContext *context;
     ...
     return prophecy_input_switch_select( context->input_switch, UDCI_KEYBOARD, window->channel );
}
                   ...
                   DFBInputDeviceModifierMask  modifiers )
{
     ...
     prophecy_input_switch_unset_filter( context->input_switch, key->filter );
...
ReactionResult
_prophecy_window_input_channel_listener( const void *msg_data,
                                       void       *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Window, "_prophecy_window_input_channel_listener( %p, %p )\n",
                 event, window );
               ...
               bool                 complete )
{
     ...
     if (complete || stret_region_visible( window->region, region, true, regions, 32, &num_regions )) {
          if (region) {
               ...
               prophecy_context_update( window->context, &area, 1, flags );
          }
          else {
          ...
          prophecy_context_update( window->context, regions, num_regions, flags );
             ...
             int           dy )
{
     ...
     prophecy_window_post_event( window, &evt );
               ...
               int           height )
{
     ...
     prophecy_input_switch_update( context->input_switch, window->channel );
     //prophecy_context_update_focus( context );
