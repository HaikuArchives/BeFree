...
D_DEBUG_DOMAIN( Prophecy_Context, "Prophecy/Context", "Prophecy's Stack Context" );


static const ReactionFunc prophecy_context_globals[] = {
     _prophecy_wm_module_context_listener,
     NULL
};
...
context_destructor( FusionObject *object, bool zombie, void *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Context, "destroying %p (stack %p)%s\n",
                 context, context->stack, zombie ? " (ZOMBIE)" : "");

     D_ASSUME( fusion_vector_is_empty( &context->windows ) );

     prophecy_context_notify( context, UCNF_DESTROYED );

     prophecy_device_detach_global( context->devices[UDCI_POINTER], &context->cursor_reaction );


     prophecy_input_switch_drop( context->input_switch, context->foo_channel );

     prophecy_input_channel_destroy( context->foo_channel );

     prophecy_input_switch_destroy( context->input_switch );


     for (i=0; i<_UDCI_NUM; i++)
          prophecy_device_destroy( context->devices[i] );


     while (fusion_vector_has_elements( &context->windows )) {
          prophecy_window_destroy( fusion_vector_at( &context->windows, 0 ) );
     }
...
FusionObjectPool *
prophecy_context_pool_create( const FusionWorld *world )
{
     return fusion_object_pool_create( "Prophecy Context Pool", sizeof(UniqueContext),
                ...
                void            *ctx )
{
     ...
     prophecy_device_connect( device, source );
                ...
                WMShared      *shared )
{
     ...
     for (i=0; i<_UDCI_NUM; i++) {
          ...
          ret = prophecy_device_create( core, context, shared->device_classes[i],
                                      context, &context->devices[i] );
          if (ret)
               goto error;

          ret = prophecy_input_switch_add( context->input_switch, context->devices[i] );
          if (ret)
               goto error_add;
...
DFBResult
prophecy_context_create( CoreDFB            *core,
                       ...
                       UniqueContext     **ret_context )
{
     ...
     context = prophecy_wm_create_context();
     if (!context)
          return DFB_FUSION;
     ...
     ret = prophecy_input_switch_create( context, &context->input_switch );
     if (ret)
          goto error_switch;
     ...
     ret = prophecy_input_channel_create( core, context, &context->foo_channel );
     if (ret)
          goto error_foo_channel;

     ret = prophecy_device_attach_global( context->devices[UDCI_POINTER],
...
DFBResult
prophecy_context_notify( UniqueContext                  *context,
                       UniqueContextNotificationFlags  flags )
{
     ...
     return prophecy_context_dispatch( context, &notification, prophecy_context_globals );
}

DFBResult
prophecy_context_set_active( UniqueContext  *context,
                           bool            active )
{
     D_MAGIC_ASSERT( context, UniqueContext );
     ...
     return prophecy_context_flush_keys( context );
}

DFBResult
prophecy_context_set_color( UniqueContext  *context,
...
void
prophecy_draw_cursor( CoreWindowStack *stack, UniqueContext *context, CardState *state, DFBRegion *region )
{
     DFBRectangle            src;
...
DFBResult
prophecy_context_update( UniqueContext       *context,
                       ...
                       DFBSurfaceFlipFlags  flags )
{
     ...
     D_DEBUG_AT( Prophecy_Context, "prophecy_context_update( num %d, flags 0x%08x )\n", num, flags );
     ...
     for (i=0; i<num; i++) {
          ...
          D_DEBUG_AT( Prophecy_Context, "    (%2d) %4d, %4d - %4dx%4d\n", i,
                      DFB_RECTANGLE_VALS_FROM_REGION( &regions[count-1] ) );
     }

     if (!count) {
          D_DEBUG_AT( Prophecy_Context, "    -> No intersection with root!\n" );
          return DFB_OK;
     }
     ...
     for (i=0; i<count; i++) {
          ...
          if (context->cursor_drawn && dfb_region_region_intersect( &cursor_inter, update )) {
               ...
               prophecy_draw_cursor( context->stack, context, state, &cursor_inter );
          }
     }
...
DFBResult
prophecy_context_resize( UniqueContext *context,
                       int            width,
                       int            height )
{
     D_MAGIC_ASSERT( context, UniqueContext );
...
DFBResult
prophecy_context_flush_keys( UniqueContext *context )
{
     D_MAGIC_ASSERT( context, UniqueContext );
...
DFBResult
prophecy_context_window_at( UniqueContext  *context,
                          int             x,
                          int             y,
...
DFBResult
prophecy_context_lookup_window( UniqueContext  *context,
...
DFBResult
prophecy_context_enum_windows( UniqueContext        *context,
...
ReactionResult
_prophecy_cursor_device_listener( const void *msg_data,
                                void       *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Context, "_prophecy_cursor_device_listener( %p, %p )\n", event, context );
