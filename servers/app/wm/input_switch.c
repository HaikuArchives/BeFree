...
D_DEBUG_DOMAIN( Prophecy_InpSw, "Prophecy/InpSwitch", "Prophecy's Input Switch" );
...
DFBResult
prophecy_input_switch_create( UniqueContext      *context,
                            UniqueInputSwitch **ret_switch )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_create( context %p )\n", context );
     ...
     D_DEBUG_AT( Prophecy_InpSw, "  -> input_switch created (%p)\n", input_switch );
...
DFBResult
prophecy_input_switch_destroy( UniqueInputSwitch *input_switch )
{
     int               i;
     DirectLink       *n;
     SwitchConnection *connection;
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_destroy( %p )\n", input_switch );
     ...
     for (i=0; i<_UDCI_NUM; i++) {
          ...
          direct_list_foreach_safe (filter, n, target->filters) {
               ...
               D_DEBUG_AT( Prophecy_InpSw, "  -> filter %p, index %d, channel %p\n",
...
DFBResult
prophecy_input_switch_add( UniqueInputSwitch *input_switch,
                         UniqueDevice      *device )
{
     DFBResult         ret;
     SwitchConnection *connection;
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_add( %p, %p )\n", input_switch, device );
     ...
     ret = prophecy_device_attach_global( device, PROPHECY_INPUT_SWITCH_DEVICB_LISTENER,
                                        input_switch, &connection->reaction );
     if (ret) {
          D_DERROR( ret, "Prophecy/InpSwitch: Could not attach global device reaction!\n" );
...
DFBResult
prophecy_input_switch_remove( UniqueInputSwitch *input_switch,
                            UniqueDevice      *device )
{
     SwitchConnection *connection;
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_remove( %p, %p )\n", input_switch, device );
...
DFBResult
prophecy_input_switch_select( UniqueInputSwitch      *input_switch,
                            UniqueDeviceClassIndex  index,
                            UniqueInputChannel     *channel )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_select( %p, %d, %p )\n",
...
DFBResult
prophecy_input_switch_set( UniqueInputSwitch      *input_switch,
                         UniqueDeviceClassIndex  index,
                         UniqueInputChannel     *channel )
{
     UniqueInputTarget *target;
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_set( %p, %d, %p )\n",
                 input_switch, index, channel );
...
DFBResult
prophecy_input_switch_unset( UniqueInputSwitch      *input_switch,
                           UniqueDeviceClassIndex  index,
                           UniqueInputChannel     *channel )
{
     UniqueInputTarget *target;

     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_unset( %p, %d, %p )\n",
                 input_switch, index, channel );
...
DFBResult
prophecy_input_switch_set_filter( UniqueInputSwitch       *input_switch,
                                UniqueDeviceClassIndex   index,
                                UniqueInputChannel      *channel,
                                ...
                                UniqueInputFilter      **ret_filter )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_set_filter( %p, %d, %p, %p )\n",
     ...
     direct_list_foreach (filter, target->filters) {
          ...
          if (prophecy_device_filter( target->clazz, event, &filter->filter ))
               return DFB_BUSY;
     }
...
DFBResult
prophecy_input_switch_unset_filter( UniqueInputSwitch *input_switch,
                                  UniqueInputFilter *filter )
{
     UniqueInputTarget *target;
     UniqueContext     *context;

     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_unset_filter( %p, %p )\n",
                 input_switch, filter );
...
DFBResult
prophecy_input_switch_drop( UniqueInputSwitch  *input_switch,
                          UniqueInputChannel *channel )
{
     int            i;
     UniqueContext *context;

     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_drop( %p, %p )\n", input_switch, channel );
     ...
     for (i=0; i<_UDCI_NUM; i++) {
          ...
          D_DEBUG_AT( Prophecy_InpSw, "  -> index %d, filters %p\n", i, target->filters );

          direct_list_foreach_safe (filter, n, target->filters) {
               D_MAGIC_ASSERT( filter, UniqueInputFilter );
               D_MAGIC_ASSERT( filter->channel, UniqueInputChannel );

               D_DEBUG_AT( Prophecy_InpSw,
...
DFBResult
prophecy_input_switch_update( UniqueInputSwitch  *input_switch,
                            UniqueInputChannel *channel )
{
     int            x, y, i;
     StretRegion   *region;
     ...
     D_DEBUG_AT( Prophecy_InpSw, "prophecy_input_switch_update( %d, %d )\n", x, y );
               ...
               UniqueInputChannel     *channel )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "target_switch( index %d, x %d, y %d, channel %p )\n",
     ...
     if (current) {
          ...
          prophecy_input_channel_dispatch( current, &evt );
     }
     ...
     if (channel) {
          ...
          prophecy_input_channel_dispatch( channel, &evt );
     }
                 ...
                 const UniqueInputEvent *event )
{
     ...
     if (channel)
          prophecy_input_channel_dispatch( channel, event );
     else
          D_DEBUG_AT( Prophecy_InpSw, "target_dispatch( class %d ) "
...
update_targets( UniqueInputSwitch *input_switch )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "update_targets( %d, %d )\n", x, y );
...
ReactionResult
_prophecy_input_switch_device_listener( const void *msg_data,
                                      void       *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "_prophecy_input_switch_device_listener( %p, %p )\n",
                 event, inpsw );
     ...
     switch (event->type) {
          ...
          case UIET_BUTTON:
               ...
               if (event->pointer.press && !target->implicit) {
                    D_DEBUG_AT( Prophecy_InpSw, "  -> implicit pointer grab, %p\n", target->current );
               ...
               if (!event->pointer.press && !event->pointer.buttons) {
                    ...
                    if (target->implicit) {
                         D_DEBUG_AT( Prophecy_InpSw, "  -> implicit pointer ungrab, %p\n", target->implicit );
          ...
          case UIET_KEY:
               ...
               if (event->keyboard.press && !target->implicit) {
                    D_DEBUG_AT( Prophecy_InpSw, "  -> implicit keyboard grab, %p\n", target->current );
               ...
               if (!event->keyboard.press && !event->keyboard.modifiers) {
                    ...
                    if (target->implicit) {
                         D_DEBUG_AT( Prophecy_InpSw, "  -> implicit keyboard ungrab, %p\n", target->implicit );
                  ...
                  SwitchConnection  *connection )
{
     ...
     D_DEBUG_AT( Prophecy_InpSw, "purge_connection( %p, %p )\n", input_switch, connection );
     ...
     prophecy_device_detach_global( connection->device, &connection->reaction );
