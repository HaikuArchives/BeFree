...
#include "stret.h"
#include "stret_iteration.h"
#include "internal.h"


#define MAX_CLASSES 16

D_DEBUG_DOMAIN( Prophecy_StReT, "Prophecy/StReT", "Prophecy's Stack Region Tree" );
...
default_update( StretRegion     *region,
                ...
                int              num )
{
     D_DEBUG_AT( Prophecy_StReT, "default_update( %p, %p, %p, %lu, %d, %d, %p, %d )\n",
                      ...
                      StretRegionClassID     *ret_id )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "stret_class_register( %p )\n", clazz );
     ...
     D_DEBUG_AT( Prophecy_StReT, "    -> New class ID is %d.\n", i );
...
{
     D_DEBUG_AT( Prophecy_StReT, "stret_class_unregister( %d )\n", id );
                     ...
                     StretRegion        **ret_region )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "stret_region_create( class %d, flags 0x%08x, %d,%d - %dx%d (%d), "
...
#if D_DEBUG_ENABLED
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "    -> Index %d, absolute bounds: %d,%d - %dx%d\n",
          ...
          int             y2 )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "  clip_out( %4d, %4d - %4dx%4d )\n",
     ...
     while (true) {
          ...
          if (!region || region == context->region) {
               ...
               if (context->num <= context->max) {
                    ...
                    D_DEBUG_AT( Prophecy_StReT, "    (%2d) %4d, %4d - %4dx%4d\n",
                    ...
                    D_DEBUG_AT( Prophecy_StReT, "  Maximum number of regions exceeded, dropping...\n" );
                      ...
                      int             *ret_num )
{
     ...
     if (! D_FLAGS_IS_SET( region->flags, SRF_ACTIVE )) {
          D_DEBUG_AT( Prophecy_StReT, "  -> Region is not active and therefore invisible!\n" );
     ...
     if (region->parent) {
          ...
          do {
               ...
               if (! D_FLAGS_IS_SET( parent->flags, SRF_ACTIVE )) {
                    D_DEBUG_AT( Prophecy_StReT, "  -> At least one parent is not active!\n" );
     ...
     if (!visible) {
          D_DEBUG_AT( Prophecy_StReT, "  -> Region is fully clipped by ancestors!\n" );
     ...
     *ret_num = context.num;
     ...
     if (context.num > context.max) {
          D_DEBUG_AT( Prophecy_StReT, "  -> Failed with %d/%d regions!\n",
                      context.num, context.max );

          return DFB_LIMITEXCEEDED;
     }

     D_DEBUG_AT( Prophecy_StReT, "  -> Succeeded with %d/%d regions.\n", context.num, context.max );
               ...
               int            y2 )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "  region_update( %4d, %4d - %4dx%4d )\n",
     ...
     D_DEBUG_AT( Prophecy_StReT, "    -> %4d, %4d - %4dx%4d  @ %4d, %4d  (class %d, index %d)\n",
                 DFB_RECTANGLE_VALS_FROM_REGION( &region->bounds ),
                     ...
                     void            *update_data )
{
     ...
     if (clip && !dfb_region_region_intersect( &area, clip )) {
          D_DEBUG_AT( Prophecy_StReT, "  -> Region doesn't intersect with clip.\n" );
          return DFB_OK;
     }
                 ...
                 StretRegionClassID  class_id )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "stret_region_at( %p, %d, %d, 0x%08x )\n", region, x, y, flags );

     if (! D_FLAGS_IS_SET( region->flags, SRF_ACTIVE )) {
          D_DEBUG_AT( Prophecy_StReT, "  -> Region is not active.\n" );
          return NULL;
     }
