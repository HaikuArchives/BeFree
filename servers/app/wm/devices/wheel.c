...
D_DEBUG_DOMAIN( Prophecy_Wheel, "Prophecy/Wheel", "Prophecy's Wheel Device Class" );
                  ...
                  void            *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Wheel, "wheel_initialize( %p, %p, %p )\n", device, data, ctx );
                ...
                void            *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Wheel, "wheel_shutdown( %p, %p, %p )\n", device, data, ctx );
                 ...
                 CoreInputDevice     *source )
{
     ...
     D_DEBUG_AT( Prophecy_Wheel, "wheel_connected( %p, %p, %p, %p )\n",
                    ...
                    CoreInputDevice     *source )
{
     ...
     D_DEBUG_AT( Prophecy_Wheel, "wheel_disconnected( %p, %p, %p, %p )\n",
                 device, data, ctx, source );
}
                     ...
                     const DFBInputEvent *event )
{
     ...
     D_DEBUG_AT( Prophecy_Wheel, "wheel_process_event( %p, %p, %p, %p ) <- type 0x%08x\n",
...
const UniqueDeviceClass prophecy_wheel_device_class = {
