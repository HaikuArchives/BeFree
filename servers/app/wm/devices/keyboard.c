...
D_DEBUG_DOMAIN( Prophecy_Keyboard, "Prophecy/Keyboard", "Prophecy's Keyboard Device Class" );
                     ...
                     void            *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_initialize( %p, %p, %p )\n", device, data, ctx );
                   ...
                   void            *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_shutdown( %p, %p, %p )\n", device, data, ctx );
                    ...
                    CoreInputDevice     *source )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_connected( %p, %p, %p, %p )\n",
                       ...
                       CoreInputDevice     *source )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_disconnected( %p, %p, %p, %p )\n",
                 device, data, ctx, source );
}
                        ...
                        const DFBInputEvent *event )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_process_event( %p, %p, %p, %p ) <- type 0x%08x\n",
     ...
     switch (event->type) {
          ...
          case DIET_KEYRELEASE:
               ...
               prophecy_device_dispatch( device, &evt );
               break;
                       ...
                       const UniqueInputEvent *filter )
{
     ...
     D_DEBUG_AT( Prophecy_Keyboard, "keyboard_filter_event( %p, %p )\n", event, filter );
...
const UniqueDeviceClass prophecy_keyboard_device_class = {
