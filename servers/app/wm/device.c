...
D_DEBUG_DOMAIN( Prophecy_Device, "Prophecy/Device", "Prophecy's Devices" );


static const ReactionFunc prophecy_device_globals[] = {
     _prophecy_input_switch_device_listener,
     _prophecy_cursor_device_listener,
     NULL
};
...
DFBResult
prophecy_device_class_register( const UniqueDeviceClass *clazz,
                              UniqueDeviceClassID     *ret_id )
{
     int i;

     D_DEBUG_AT( Prophecy_Device, "prophecy_device_class_register( %p )\n", clazz );
     ...
     D_DEBUG_AT( Prophecy_Device, "    -> New class ID is %d.\n", i );
...
DFBResult
prophecy_device_class_unregister( UniqueDeviceClassID id )
{
     D_DEBUG_AT( Prophecy_Device, "prophecy_device_class_unregister( %d )\n", id );
...
DFBResult
prophecy_device_create( CoreDFB              *core,
                      ...
                      UniqueDevice        **ret_device )
{
     ...
     D_DEBUG_AT( Prophecy_Device, "prophecy_device_create( class %d )\n", class_id );
     ...
     device->reactor = fusion_reactor_new( sizeof(UniqueInputEvent),
                                           "Prophecy Device", dfb_core_world(core) );
     if (!device->reactor) {
          ret = DFB_FUSION;
          goto error;
     }
     ...
     D_DEBUG_AT( Prophecy_Device, "  -> device created (%p)\n", device );
...
DFBResult
prophecy_device_destroy( UniqueDevice *device )
{
     DirectLink              *n;
     DeviceConnection        *connection;
     UniqueContext           *context;
     ...
     D_DEBUG_AT( Prophecy_Device, "prophecy_device_destroy( %p )\n", device );
...
DFBResult
prophecy_device_connect( UniqueDevice    *device,
                       CoreInputDevice *source )
{
     ...
     D_DEBUG_AT( Prophecy_Device, "prophecy_device_connect( %p, %p, ID %d )\n",
...
DFBResult
prophecy_device_disconnect( UniqueDevice    *device,
                          CoreInputDevice *source )
{
...
DFBResult
prophecy_device_attach( UniqueDevice *device,
                      ReactionFunc  func,
                      void         *ctx,
                      Reaction     *reaction )
{
...
DFBResult
prophecy_device_detach( UniqueDevice *device,
                      Reaction     *reaction )
{
     D_MAGIC_ASSERT( device, UniqueDevice );
...
DFBResult
prophecy_device_attach_global( UniqueDevice   *device,
                             int             index,
                             void           *ctx,
                             GlobalReaction *reaction )
{
...
DFBResult
prophecy_device_detach_global( UniqueDevice   *device,
                             GlobalReaction *reaction )
{
     D_MAGIC_ASSERT( device, UniqueDevice );
...
DFBResult
prophecy_device_dispatch( UniqueDevice           *device,
                        const UniqueInputEvent *event )
{
     D_MAGIC_ASSERT( device, UniqueDevice );

     return fusion_reactor_dispatch( device->reactor, event, true, prophecy_device_globals );
}

bool
prophecy_device_filter ( UniqueDeviceClassID     class_id,
                       const UniqueInputEvent *event,
...
ReactionResult
_prophecy_device_listener( const void *msg_data,
                         void       *ctx )
{
