...
D_DEBUG_DOMAIN( WM_Unique, "WM/Prophecy", "Prophecy - BeFree Window Manager" );


DFB_WINDOW_MANAGER( prophecy );
...
initialize_data( CoreDFB *core, WMData *data, WMShared *shared )
{
     ...
     data->shared     = shared;
     data->module_abi = PROPHECY_WM_ABI_VERSION;
...
wm_get_info( CoreWMInfo *info )
{
     ...
     info->version.binary = PROPHECY_WM_ABI_VERSION;

     snprintf( info->name, DFB_CORE_WM_INFO_NAME_LENGTH, "Prophecy" );
...
wm_initialize( CoreDFB *core, void *wm_data, void *shared_data )
{
     ...
     return prophecy_wm_module_init( core, data, shared, true );
}
...
wm_join( CoreDFB *core, void *wm_data, void *shared_data )
{
     ...
     return prophecy_wm_module_init( core, data, shared, false );
}
...
wm_shutdown( bool emergency, void *wm_data, void *shared_data )
{
     ...
     prophecy_wm_module_deinit( wm_data, shared_data, true, emergency );
...
wm_leave( bool emergency, void *wm_data, void *shared_data )
{
     ...
     prophecy_wm_module_deinit( wm_data, shared_data, false, emergency );
               ...
               void            *stack_data )
{
     ...
     if (ret) {
          D_DERROR( ret, "WM/Prophecy: Could not get the primary region!\n" );
          return ret;
     }

     /* Create the prophecy context. */
     ret = prophecy_context_create( wmdata->core, stack, region, context->layer_id,
                                  wmdata->shared, &data->context );
     dfb_layer_region_unref( region );
     if (ret) {
          D_DERROR( ret, "WM/Prophecy: Could not create the context!\n" );
          return ret;
     }

     /* Attach the global context listener. */
     ret = prophecy_context_attach_global( data->context,
                                         PROPHECY_WM_MODULB_CONTEXT_LISTENER,
                                         data, &data->context_reaction );
     if (ret) {
          prophecy_context_unref( data->context );
          D_DERROR( ret, "WM/Prophecy: Could not attach global context listener!\n" );
          return ret;
     }

     /* Inherit all local references from the layer context. */
     ret = prophecy_context_inherit( data->context, context );
     prophecy_context_unref( data->context );
     if (ret) {
          prophecy_context_detach_global( data->context, &data->context_reaction );
          D_DERROR( ret, "WM/Prophecy: Could not inherit from layer context!\n" );
          return ret;
     }
               ...
               bool             active )
{
     ...
     return prophecy_context_set_active( data->context, active );
}
                 ...
                 int              height )
{
     ...
     return prophecy_context_resize( data->context, width, height );
}
               ...
               void            *stack_data )
{
     ...
     return prophecy_context_flush_keys( data->context );
}
              ...
              CoreWindow      **ret_window )
{
     ...
     ret = prophecy_context_window_at( data->context, x, y, &window );
     if (ret)
          return ret;
                  ...
                  CoreWindow      **ret_window )
{
     ...
     ret = prophecy_context_lookup_window( data->context, window_id, &window );
     if (ret)
          return ret;
                 ...
                 void                 *callback_ctx )
{
     ...
     return prophecy_context_enum_windows( data->context, callback, callback_ctx );
}
               ...
               void            *window_data )
{
     ...
     /* Create the prophecy window. */
     ret = prophecy_window_create( wmdata->core, window, data->context,
                                 window->caps, &window->config, &data->window );
     if (ret) {
          D_DERROR( ret, "WM/Prophecy: Could not create window!\n" );
          return ret;
     }

     /* Attach the global window listener. */
     ret = prophecy_window_attach_global( data->window,
                                        PROPHECY_WM_MODULB_WINDOW_LISTENER,
                                        data, &data->window_reaction );
     if (ret) {
          prophecy_window_unref( data->window );
          D_DERROR( ret, "WM/Prophecy: Could not attach global window listener!\n" );
          return ret;
     }

     /* Inherit all local references from the layer window. */
     ret = prophecy_window_inherit( data->window, window );
     prophecy_window_unref( data->window );
     if (ret) {
          prophecy_window_detach_global( data->window, &data->window_reaction );
          D_DERROR( ret, "WM/Prophecy: Could not inherit from core window!\n" );
          return ret;
     }

     prophecy_window_get_config( data->window, &window->config );
                      ...
                      CoreWindowConfigFlags   flags )
{
     ...
     ret = prophecy_window_set_config( data->window, config, flags );
                   ...
                   int                     relation )
{
     ...
     return prophecy_window_restack( data->window, rel_data ? rel_data->window : NULL, relation );
}
         ...
         CoreWMGrab *grab )
{
     ...
     return prophecy_window_grab( data->window, grab );
}
           ...
           CoreWMGrab *grab )
{
     ...
     return prophecy_window_ungrab( data->window, grab );
}
                  ...
                  void       *window_data )
{
     ...
     return prophecy_window_request_focus( data->window );
}
                 ...
                 DFBSurfaceFlipFlags  flags )
{
     ...
     return prophecy_context_update( data->context, region, 1, flags );
}
                  ...
                  DFBSurfaceFlipFlags  flags )
{
     ...
     return prophecy_window_update( data->window, region, flags );
}
                  ...
                  CoreCursorUpdateFlags  flags )
{
     ...
     if (flags & CCUF_DISABLE) {
          ...
          /* draw cursor */
          prophecy_draw_cursor( stack, context, state, &context->cursor_region );
