...
D_DEBUG_DOMAIN( Prophecy_Window, "Prophecy/WindowReg", "Prophecy's Window Region Class" );
                  ...
                  UniqueInputChannel **ret_channel )
{
     ...
     D_DEBUG_AT( Prophecy_Window, "window_get_input( region %p, window %p, index %d, x %d, y %d )\n",
               ...
               int              num )
{
     ...
     D_DEBUG_AT( Prophecy_Window, "window_update( region %p, window %p, visible %s, num %d )\n",
                 region, window, visible ? "yes" : "no", num );
#if D_DEBUG_ENABLED
     for (i=0; i<num; i++) {
          D_DEBUG_AT( Prophecy_Window, "    (%d)  %4d,%4d - %4dx%4d\n",
...
const StretRegionClass prophecy_window_region_class = {
