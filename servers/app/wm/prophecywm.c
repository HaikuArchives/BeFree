...
#include "internal.h"
#include "prophecywm.h"
#include "data/foo.h"


D_DEBUG_DOMAIN( Prophecy_WM, "Prophecy/WM", "Prophecy - BeFree Window Manager" );
...
static const StretRegionClass *region_classes[_URCI_NUM] = {
     &prophecy_root_region_class,
     &prophecy_frame_region_class,
     &prophecy_window_region_class,
     &prophecy_foo_region_class
};
...
static const UniqueDeviceClass *device_classes[_UDCI_NUM] = {
     &prophecy_pointer_device_class,
     &prophecy_wheel_device_class,
     &prophecy_keyboard_device_class
};
...
load_foo( CoreDFB *core, WMShared *shared )
{
     ...
     if (ret) {
          D_DERROR( ret, "Prophecy/WM: Could not create %dx%d surface for border tiles!\n",
     ...
     if (ret) {
          D_DERROR( ret, "Prophecy/WM: Could not lock surface for border tiles!\n" );
...
DFBResult
prophecy_wm_module_init( CoreDFB *core, WMData *data, WMShared *shared, bool master )
{
     DFBResult ret;

     D_DEBUG_AT( Prophecy_WM, "prophecy_wm_init( core %p, data %p, shared %p, %s )\n",
     ...
     if (data->module_abi != PROPHECY_WM_ABI_VERSION) {
          D_ERROR( "Prophecy/WM: Module ABI version (%d) does not match %d!\n",
     ...
     if (master) {
          ...
          ret = dfb_input_add_global( _prophecy_device_listener, &shared->device_listener );
          if (ret)
               goto error_global;

          shared->context_pool    = prophecy_context_pool_create( data->world );
          shared->decoration_pool = prophecy_decoration_pool_create( data->world );
          shared->window_pool     = prophecy_window_pool_create( data->world );
          ...
          dfb_input_set_global( _prophecy_device_listener, shared->device_listener );
...
void
prophecy_wm_module_deinit( WMData *data, WMShared *shared, bool master, bool emergency )
{
     D_DEBUG_AT( Prophecy_WM, "prophecy_wm_deinit( %s%s ) <- core %p, data %p, shared %p\n",
...
ReactionResult
_prophecy_wm_module_context_listener( const void *msg_data,
                                    void       *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_WM, "%s( context %p, flags 0x%08x )\n",
...
ReactionResult
_prophecy_wm_module_window_listener ( const void *msg_data,
                                    void       *ctx )
{
     ...
     D_DEBUG_AT( Prophecy_WM, "%s( window %p, flags 0x%08x )\n",
...
DirectResult
prophecy_wm_enum_contexts( FusionObjectCallback  callback,
                         void                 *ctx )
{
     D_ASSERT( dfb_core != NULL );
     D_ASSERT( wm_data != NULL );
...
DirectResult
prophecy_wm_enum_windows( FusionObjectCallback  callback,
                        void                 *ctx )
{
     D_ASSERT( dfb_core != NULL );
     D_ASSERT( wm_data != NULL );
                         ...
                         bool      master )
{
     ...
     for (i=0; i<_URCI_NUM; i++) {
          ...
          if (ret) {
               D_DERROR( ret, "Prophecy/WM: Failed to register region class %d!\n", i );
          ...
          if (master)
               ...
               D_ERROR( "Prophecy/WM: Class IDs mismatch (%d/%d)!\n",
                         ...
                         bool      master )
{
     ...
     for (i=0; i<_UDCI_NUM; i++) {
          ret = prophecy_device_class_register( device_classes[i], &class_id );
          if (ret) {
               D_DERROR( ret, "Prophecy/WM: Failed to register device class %d!\n", i );
          ...
          if (master)
               ...
               prophecy_device_class_unregister( class_id );
...
error:
     while (--i >= 0)
          prophecy_device_class_unregister( shared->device_classes[i] );
