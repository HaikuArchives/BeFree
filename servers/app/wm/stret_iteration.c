...
stret_iteration_init( StretIteration *iteration, StretRegion *region, StretRegion *abort_at )
{
     ...
     D_DEBUG_AT( Prophecy_StReT, "stret_iteration_init()\n" );
     ...
     do {
          ...
          D_DEBUG_AT( Prophecy_StReT, "  -> (%d) %p, last level %d, last index %d\n",
                      ...
                      const DFBRegion *clip )
{
     ...
     if (clip)
          D_DEBUG_AT( Prophecy_StReT, "stret_iteration_next( %d, %d - %dx%d )\n",
                      DFB_RECTANGLE_VALS_FROM_REGION( clip ) );
     else
          D_DEBUG_AT( Prophecy_StReT, "stret_iteration_next()\n" );
     ...
     while (iteration->frame >= 0) {
          ...
          D_DEBUG_AT( Prophecy_StReT, "  -> (%d) %p, level [%d/%d], index %d\n",
